...
exports.getF4Data = ({
  ...
  const P51 = Math.round(
    (width - (jointCoverOffset ? 42 : 60) - (traverse63 ? 69 : 120)) / 4
  );
  const P52 = height - (jointCoverOffset ? 42 : 60);
  const P66 = Math.round((P52 - 5) / 120);

  const R51 = (quantity * 8 * P51) / 1000;
  const R52 = (quantity * 8 * P52) / 1000;
  ...
  return {
    profiles: [
...

      {
        ...
        color,
        param: "Width",
        quantity: glazzingVal === 9 ? quantity * 4 : 0,
        length: P51 - 5,
      },
    ],
    accessories: [
      ...
      },
      {
        ...
        color: "",
        quantity: Math.round([5, 6, 9].includes(glazzingVal) ? S50 : 0),
      },
    ],
    glazzingValues: [
      {
