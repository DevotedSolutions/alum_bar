...
exports.getAllproduct = async (req, res) => {
  try {
    ...
    let getdata = await productSchema
      .find()
      .sort({ productcode: 1 })
      .skip(skip)
      .limit(size);
...
exports.getTotalRevenue = async (req, res) => {
  try {
    ...
    const currentMonthSales = monthlySales.find(
      (month) => month.month === currentMonth
    ) || { month: currentMonth, total: 0 };

    const formattedMonthlySales = Array.from({ length: 12 }, (_, i) => {
      const monthData = monthlySales.find((month) => month.month === i + 1) || {
        month: i + 1,
        total: 0,
      };
      return monthData;
    ...
    res
      .status(500)
      .json({ error: "Failed to retrieve total revenue and monthly sales" });
  }
};
